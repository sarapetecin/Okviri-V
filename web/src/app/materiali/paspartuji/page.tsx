...
function oblikujCeno(znesek: number | null) {
    if (znesek === null) {
        return "—";
    }
...
export default async function PaspartujiPage({
    ...
    const sporociloNapake =
        napaka === "brisanje"
            ? "Paspartuja ni bilo mogoče izbrisati."
            : null;
    const sporociloUspeha =
        uspeh === "izbrisano"
            ? "Paspartu je bil izbrisan iz kataloga."
            : null;
        ...
        <main className="min-h-screen bg-slate-100">
            <header className="border-b border-slate-200 bg-white">
                <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
                    ...
                    <Link href="/materiali" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50">
                        Nazaj na katalog
                    </Link>
                </div>
            </header>
            ...
            <section className="mx-auto max-w-7xl px-6 py-10">
                <div className="mb-7 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
                    <div>
                        ...
                        <p className="mt-2 text-slate-600">
                            V katalogu je {paspartuji?.length ?? 0} paspartujev.
                        </p>
                    </div>
                    <Link href="/materiali/paspartuji/nov" className="rounded-lg bg-slate-900 px-5 py-2.5 text-center font-semibold text-white transition hover:bg-slate-700">
                        Dodaj paspartu
                    </Link>
                </div>
                ...
                <form className="mb-6 flex flex-col gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row">
                    ...
                    {iskaniNiz && (
                        <Link href="/materiali/paspartuji" className="rounded-lg border border-slate-300 px-5 py-2.5 text-center font-medium text-slate-700">
                            Počisti
                        </Link>
                    )}
                </form>
                            ...
                            <table className="w-full text-left">
                                <thead className="border-b border-slate-200 bg-slate-50">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-right text-sm font-semibold text-slate-700">Dejanja</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-200">
                                    {(paspartuji ?? []).map((paspartu) => (
                                        <tr key={paspartu.id} className="hover:bg-slate-50">
                                            ...
                                            <td className="px-4 py-3">
                                                <span className={paspartu.na_prodaj ? "rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-800" : "rounded-full bg-slate-200 px-2.5 py-1 text-xs font-semibold text-slate-700"}>
                                                    {paspartu.na_prodaj ? "Na prodaj" : "Neaktiven"}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3">
                                                <div className="flex justify-end gap-2">
                                                    <Link href={`/materiali/paspartuji/${paspartu.id}/uredi`} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                                                        Uredi
                                                    </Link>
                                                    <GumbIzbrisiMaterial naziv={paspartu.oznaka ?? paspartu.naziv} action={izbrisiMaterial.bind(null, "paspartu", paspartu.id)} />
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
